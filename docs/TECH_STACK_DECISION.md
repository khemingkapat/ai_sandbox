...
**Decision:** We will adopt **Option 1: Dual-Path Execution**. Interactive web-based workloads (Jupyter, VS Code) will run as native OCI containers managed by Slinky's `slurm-bridge` and Kubernetes Pods. Batch workloads (heavy AI training, parameter searches) will be submitted as raw bash scripts directly to Slurm and execute natively on the bare-metal worker nodes, leveraging globally provisioned environments (such as Conda, environment modules, or shared Apptainer `.sif` files on NFS).
**Rationale:** While a pure OCI-only approach creates a highly unified codebase, it fundamentally forces batch jobs into Kubernetes Pods. This destroys the ability to perform high-throughput native Slurm array submissions and introduces the exact "fake hardware" blocking limitation when testing locally. By adopting Dual-Path Execution, we perfectly mirror enterprise HPC environments: using modern cloud-native Kubernetes purely for user-facing interactive gateways, while preserving the raw, unencumbered speed and topology awareness of native Slurm execution for heavy batch workloads.
**Consequences:** The Go Portal will maintain its two-pronged submission logic in `handlers.go`. Interactive jobs will utilize the `SessionManager` to spawn Kubernetes Pods. Batch jobs will bypass Kubernetes entirely and construct raw `sbatch` scripts (utilizing `apptainer exec` or `source activate`) to be submitted to the Slurm REST API. Batch applications in the Portal must continue defining their environments using `image_file` or equivalent native environment flags.
**References:**
...
*   **Student Custom Inference (GPU #2):** For students testing their own custom fine-tuned models on demand, **Option A (vLLM)** is mandatory. Each student runs vLLM inside a strictly isolated, ephemeral Kubernetes pod that mounts their private NFS folder. While a single-user pod does not benefit from vLLM's high-concurrency memory features (PagedAttention), using vLLM locally is critical for **API Standardization**. It ensures that code written to test a private model locally uses the exact same OpenAI API format as the Central Triton Service, and it allows the IT team to maintain only a single standardized LLM container image across the entire platform. Triton's requirement for Ahead-Of-Time compilation is completely unfeasible for this on-demand learning environment.
**References:**
